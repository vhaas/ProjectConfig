			...
			<section>
				...
				<form class="form-horizontal" {{action save on="submit"}}>
					{{#view App.BootstrapControl inputId="name" label="Name"}}
						{{view Ember.TextField disabledBinding="controller.isEnabled" valueBinding="name"}}
					{{/view}}
					{{#view App.BootstrapControl inputId="goal" label="Ziel"}}
						{{view Ember.TextField disabledBinding="controller.isEnabled" valueBinding="goal"}}
					{{/view}}
					{{#view App.BootstrapControl inputId="benefit" label="Nutzen"}}
						{{view Ember.TextField disabledBinding="controller.isEnabled" valueBinding="benefit"}}
					{{/view}}
					{{#view App.BootstrapControl inputId="role" label="Rolle"}}
						{{view App.RoleSelect
							disabledBinding="controller.isEnabled"
					...
					{{#view App.BootstrapControl inputId="description" label="Beschreibung"}}
						{{view Ember.TextArea disabledBinding="controller.isEnabled" valueBinding="description"}}
					{{/view}}
					{{#view App.BootstrapControl inputId="willGetDeleted" label="User Story löschen"}}
						{{view Ember.Checkbox disabledBinding="controller.isEnabled" checkedBinding="willGetDeleted"}}
					{{/view}}
					{{#if willGetDeleted}}
