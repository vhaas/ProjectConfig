			...
			<table>
				<thead>
					<tr>
						...
						<g:sortableColumn property="description" title="${message(code: 'step.description.label', default: 'Description')}" />
					
						<g:sortableColumn property="name" title="${message(code: 'step.name.label', default: 'Name')}" />
					
						<g:sortableColumn property="orderId" title="${message(code: 'step.orderId.label', default: 'Order Id')}" />
				...
				<g:each in="${stepInstanceList}" status="i" var="stepInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${stepInstance.id}">${fieldValue(bean: stepInstance, field: "description")}</g:link></td>
					
						<td>${fieldValue(bean: stepInstance, field: "name")}</td>
					
						<td>${fieldValue(bean: stepInstance, field: "orderId")}</td>
